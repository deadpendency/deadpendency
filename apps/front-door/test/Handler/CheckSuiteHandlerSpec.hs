module Handler.CheckSuiteHandlerSpec (spec) where
...
import GitHub.Data.Webhooks.Events (CheckSuiteEventAction (..))
...
spec = parallel $
  context "When processing events" $ do
    ...
    it "a happy day test produces correct results" $
      hedgehog $ do
        event <- forAll genCheckSuiteEvent
        let massagedEvent =
              event
                & #evCheckSuiteAction .~ CheckSuiteEventActionRequested
                & (#evCheckSuiteInstallation . _Just . #whChecksInstallationId) .~ 12345678
                & (#evCheckSuiteCheckSuite . #whCheckSuiteHeadBranch) ?~ "git-ref"
                & (#evCheckSuiteCheckSuite . #whCheckSuiteHeadSha) .~ "git-sha"
                & (#evCheckSuiteRepository . #whRepoFullName) .~ "git-repo-dependencyname"
                & (#evCheckSuiteRepository . #whRepoName) .~ "git-repo-name"
                & (#evCheckSuiteRepository . #whRepoIsPrivate) .~ True
                & (#evCheckSuiteRepository . #whRepoOwner . _Right . #whUserLogin) .~ "git-repo-owner"
                & (#evCheckSuiteRepository . #whRepoOwner . _Right . #whUserId) .~ 1234
                & (#evCheckSuiteRepository . #whRepoNodeId) .~ "git-repo-node-id"
                & (#evCheckSuiteSender . #whUserLogin) .~ "git-user-name"
                & #evCheckSuiteOrganization .~ Nothing

        let handlerResult = checkSuiteHandler "asdf/asdf" WebhookCheckSuiteEvent ((), massagedEvent)
    ...
    it "missing installation id produces expected error" $ do
      event <- sample genCheckSuiteEvent
      let massagedEvent =
            event
              & #evCheckSuiteAction .~ CheckSuiteEventActionRequested
              & #evCheckSuiteInstallation .~ Nothing
          handlerResult = checkSuiteHandler "asdf/asdf" WebhookCheckSuiteEvent ((), massagedEvent)
          (runsCreated, (_, (_, (_, (_, (_, appErrors)))))) = runHandler handlerResult

      appErrors `shouldBe` Left (UnexpectedMissingInfoInCheckSuiteEvent "Installation Details")
    ...
    it "missing complex user produces expected error" $
      hedgehog $ do
        ...
        let massagedEvent =
              event
                & #evCheckSuiteAction .~ CheckSuiteEventActionRequested
                & (#evCheckSuiteRepository . #whRepoOwner) .~ Left simpleUser
            handlerResult = checkSuiteHandler "asdf/asdf" WebhookCheckSuiteEvent ((), massagedEvent)
            (runsCreated, (_, (_, (_, (_, (_, appErrors)))))) = runHandler handlerResult

        appErrors === Left (UnexpectedMissingInfoInCheckSuiteEvent "Repo Owner Details")
        runsCreated === []

    it "non-requested actions do not initialize a run" $
      hedgehog $ do
        event <- sample genCheckSuiteEvent
        let massagedEvent =
              event
